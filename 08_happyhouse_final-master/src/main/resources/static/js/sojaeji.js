...
//	window.onload = function() {
	...
	obj.dong.onchange = function() {
	
		//로컬스토리지에 추가
		const juso = {
			sido: obj.sido.value,
			gugun: obj.gugun.value,
			dong: obj.dong.value
		}
		
		localStorage.setItem("juso", JSON.stringify(juso));		
//		location.href='/house/search&sido2='+`${obj.sido.value}`+'&gugun2='+`${obj.gugun.value}`+'&dong2='+`${obj.dong.value}`;
		location.href='/house/search/'+`${obj.sido.value}`+'/' +`${obj.gugun.value}`+'/'+`${obj.dong.value}`;
		//location.replace("apart_result.jsp");
		var map = new kakao.maps.Map(container, options);
    var geocoder = new kakao.maps.services.Geocoder();
                               
            // 주소로 좌표를 검색합니다
            geocoder.addressSearch(`${juso.sido} ${juso.gugun} ${juso.dong}`, function(result, status) {
...
sojaeji.prototype = {
	update_gugun : function() {
		...
		for(var i=0; i<gugun.length; i++){
			//this.gugun.options.add(new Option(gugun[i], this[gugun[i]][0]));
			this.gugun.options.add(new Option(gugun[i], gugun[i]));
			//console.log(this[gugun[i]][0]);
		}
	},
	update_dong : function() {
		if (this.dong == null) return;
		//var dong = this[this.sido.value+"->"+this.gugun.options[this.gugun.selectedIndex].text];
  ...
  '시도': [ '도/광역시', '서울', '부산', '대구', '인천', '광주', '대전', '울산', '강원', '경기', '경남', '경북', '전남', '전북', '제주', '충남', '충북'],
 '도/광역시': [  '시/구/군'],
